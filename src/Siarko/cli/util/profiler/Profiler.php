...
class Profiler
{
    ...
    public static function start($id = null, ?TimeFactor $timeFactor = null)
    {
        if (!self::isEnabled()) {
            return;
        }
    ...
    public static function print()
    {
        if(self::isEnabled()){
            if (!empty(self::$logs)) {
                ...
                echo "=== Created container count: ".BaseComponent::getContainerCount()." ===\n";
    ...
    public static function getProfilerSummary(): string
    {
        ...
        foreach (self::$logs as $key => $log) {
            ...
            $tf = $log[self::TF];
            $indentCount = substr($key, strrpos($key, '_') + 1);
